...
export default function DetailRestaurant() {
  ...
  const handleSave = async () => {
    ...
    const url = saved
      ? `${API_URL}/api/saved/${user.id}/${restaurant.id}`
      : `${API_URL}/api/saved`;
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/restaurants/${restaurant.id}/menus`)
      .then(res => res.json())
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/saved/check/${user.id}/${restaurant.id}`)
      .then(res => res.json())
          ...
          <div className="flex flex-col lg:flex-row gap-12 min-h-[500px]">
            ...
            <div className="w-full lg:w-1/2 flex flex-col items-center">
              ...
              <div className="w-[420px] h-[420px] rounded-2xl shadow-xl overflow-hidden">
                <img
                  src={
                    restaurant.image?.trim().startsWith("http")
                      ? restaurant.image
