...
const getAll = async (req, res) => {
  ...
  const mapped = data.map(item => ({
    ...
    // Fix: Check if absolute URL (Unsplash) or local upload
    image: item.image 
    ...
    restaurant: {
      ...
      image: item.restaurant_image
        ? (item.restaurant_image.startsWith("http") ? item.restaurant_image : `${baseUrl}/restaurants/${item.restaurant_image}`)
        : null
    }
  }));
